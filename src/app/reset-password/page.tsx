...
export default function ResetPasswordPage() {
  ...
  if (checking) return <AuthLayout eyebrow="Secure recovery" title="Checking your reset link" description="Please wait while we verify that this password-reset link is valid."><div className="h-2 overflow-hidden rounded-full bg-[#e9f8f0]"><div className="h-full w-1/2 animate-pulse rounded-full bg-[#00a551]" /></div></AuthLayout>;
  if (!ready) return <AuthLayout eyebrow="Reset link unavailable" title="Request a new link" description="This password-reset link is invalid or has expired."><Link className={`${primaryButtonClass} block text-center`} href="/forgot-password">Send a new reset link</Link><p className="mt-5 text-center text-sm"><Link className="font-bold text-[#008c46]" href="/login">Back to sign in</Link></p></AuthLayout>;
