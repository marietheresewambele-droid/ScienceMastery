...
export interface RawPhysicsQuestionBank { metadata?: { subtopics?: Array<{ title?: string; questionCount?: number }> }; questions: RawPhysicsQuestion[]; }
...
export function createPhysicsQuestions(bank: RawPhysicsQuestionBank, topicSlug: string, topicTitle: string): MasteryQuestion[] {
  return bank.questions.map((question,index)=>{
    ...
    return { id:question.id, subject:"physics", topicSlug, topic:topicTitle, subtopic, question:question.question, marks:question.marks, assessmentObjective:ao, difficulty:tier, tier, commandWord:question.commandWord, specificationReference:question.specificationReference, markingPoints, modelAnswer:question.modelAnswer, gradeDemand:question.gradeDemand, questionFamily:question.questionFamily, originalSubtopic:question.sourceSubtopic };
  });
}
export function createPhysicsSubtopics(bank: RawPhysicsQuestionBank): BiologySubtopicConfig[] { return (bank.metadata?.subtopics ?? []).map((subtopic,index)=>({ id:(subtopic.title ?? `subtopic-${index+1}`).toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,""), title:subtopic.title ?? "Unknown subtopic", description:"Practice questions for this subtopic." })); }
export type PhysicsTopicConfig = BiologyTopicConfig;
